The base application's source code (if applicable) is available from the portable app's
...
(For this section, StickiesPortable is the executable base file name, normally
the App ID of the package.)

Some configuration in the PortableApps.com Launcher can be overridden by the
user in an INI file next to StickiesPortable.exe called StickiesPortable.ini.  If
...
PortableApps.comLauncher.ini from this directory to StickiesPortable.ini next to
StickiesPortable.exe. The options in the INI file are as follows::
